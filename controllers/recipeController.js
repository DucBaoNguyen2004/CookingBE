...
export const generateRecipe = async (req, res, next) => {
  try {
    ...
    const shouldUsePantry = usePantryIngredients || usePantry;
    const normalizedCuisineType = cuisineType.toLowerCase();
    let finalIngredients = [...ingredients];

    // Add pantry ingredients if requested
    if (shouldUsePantry) {
      const pantryItems = await PantryItem.findByUserId(req.user.id);
      if (pantryItems.length === 0 && ingredients.length === 0) {
        return res.status(400).json({
          success: false,
          message: 'Your pantry is empty. Please add some items to your pantry or manually enter ingredients.'
        });
      }
